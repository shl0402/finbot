...
def scrape_tradingview_stock_analysis(exchange: str, ticker: str, headless: bool = True) -> dict:
    ...
    with sync_playwright() as p:
        ...
        company_name = _clean_text(page.locator("h2.title-HDE_EEoW").first.inner_text()) if page.locator("h2.title-HDE_EEoW").count() > 0 else "N/A"
        exchange_name = _clean_text(page.locator("span.provider-xQqIQebY").first.inner_text()) if page.locator("span.provider-xQqIQebY").count() > 0 else "N/A"
        currency = "N/A"
